...
public class FileServiceClient : IFileServiceClient
{
    ...
    public async Task<Result<string>> GetUrlAsync(string fileName)
    {
        var response = await _httpClient.GetAsync($"{_baseUrl}/api/file/url?filename={fileName}");
        var responseContent = await response.Content.ReadAsStringAsync();

        if (!response.IsSuccessStatusCode)
        {
            _logger.LogError($"file-service: get url returned {response.StatusCode}: {responseContent}");
            return Result<string>.Failure($"Microservice request failure: {responseContent}");
        }
